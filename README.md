...
4. Update the following resources as needed.
 - find all the files that contains the word CUSTCTI and replace it with your namespace. You could run a command like this one: sed -i ``` 's/CUSTCTI/mynamespace/g' * ```
    - **Aura Bridge:** This is a sample Aura bridge component. When referring to the LMS channel, use the namespace as a prefix: `<namespace>__<lms channel name>__c`
        - In force-app/main/default/aura/AuraBridge/AuraBridge.cmp, change `<lightning:messageChannel type="CUSTCTI__ServiceCloudVoiceMessageChannel__c" ..` to `<lightning:messageChannel type="<namespace>__<lms channel name>__c" ..`
    - **Aura LMS Sample:** This is a sample Aura component that subscribes to the lightning message channel. When referring to the LMS channel, use the namespace as a prefix: `<namespace>__<lms channel name>__c`
        - In force-app/main/default/aura/AuraLmsSample/AuraLmsSample.cmp, change `<lightning:messageChannel type="CUSTCTI__ServiceCloudVoiceMessageChannel__c" ..` to `<lightning:messageChannel type="<namespace>__<lms channel name>__c"`
